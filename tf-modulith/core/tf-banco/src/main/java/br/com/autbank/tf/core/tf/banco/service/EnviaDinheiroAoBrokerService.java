...
import br.com.autbank.tf.core.tf.banco.model.apis.CadastrarEnvioBrokerRequest;
import br.com.autbank.tf.core.tf.banco.model.config.BancoConfig;
...
@AllArgsConstructor
@Slf4j
public class EnviaDinheiroAoBrokerService {

    private final BancoConfig bancoConfig;
    private final HttpClient httpClient;
    private final OAuth2Client oAuth2Client;

    public void enviar(Long idRemessa, BigDecimal valor) throws IOException, InterruptedException {
        URI uri = bancoConfig.getUriBroker();

        var requestObj = CadastrarEnvioBrokerRequest.builder()
                .idRemessa(idRemessa)
                .valor(valor)
                .build();
